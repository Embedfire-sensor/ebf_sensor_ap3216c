...
{  
  ...
  IOMUXC_SetPinMux(I2C_SCL_PIN_IOMUXC, 0U);
  IOMUXC_SetPinMux(I2C_SDA_PIN_IOMUXC, 0U); 
}
...
{
  /* 模拟IIC引脚 */    
  IOMUXC_SetPinConfig(I2C_SCL_PIN_IOMUXC,IIC_PAD_CONFIG_DATA);  
  IOMUXC_SetPinConfig(I2C_SDA_PIN_IOMUXC,IIC_PAD_CONFIG_DATA); 
}
...
void I2C_SDA_IN_Config()
{
  ...
  /* 初始化 GPIO 模拟IIC. */
  GPIO_PinInit(I2C_SDA_PIN_GPIO, I2C_SDA_PIN, &iic_config);
}
...
{     	
  ...
  iic_config.interruptMode = kGPIO_NoIntmode; //不使用中断  
  /* 初始化 GPIO 模拟IIC. */
  GPIO_PinInit(I2C_SCL_PIN_GPIO, I2C_SCL_PIN, &iic_config);
  GPIO_PinInit(I2C_SDA_PIN_GPIO, I2C_SDA_PIN, &iic_config);	
}
...
{ 
	...
	i2c_Start();
	/* 第2步：发起控制字节，高7bit是地址，bit0是读写控制位，0表示写，1表示读 */
	i2c_SendByte(IIC_ADDRESS | I2C_DIR_WR);	/* 此处是写指令 */
	/* 第3步：发送一个时钟，判断器件是否正确应答 */
	if (i2c_WaitAck() != 0)
	{
	  goto cmd_fail;	/* EEPROM器件写超时 */
...
{ 
	...
	i2c_SendByte(IIC_ADDRESS | I2C_DIR_RD);	/* 此处是读指令 */
